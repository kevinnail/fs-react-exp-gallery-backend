...
module.exports = class Post {
  ...
  static async postNewPost(
    ...
    const { rows } = await pool.query(
      ...
      num_imgs, discounted_price, original_price, sold, selling_link, hide) 
  ...
  static async addGalleryImages(post_id, image_urls, image_public_ids) {
    ...
    for (let i = 0; i < image_urls.length; i++) {
      const { rows } = await pool.query(insertQuery, [post_id, image_urls[i], image_public_ids[i]]);
  ...
  static async softDeleteById(post_id) {
    const { rows } = await pool.query(
      `UPDATE gallery_posts
  ...
  // Accepts an optional pg client so it can run inside an existing transaction
  // (the sales order create path). Falls back to the shared pool otherwise.
  static async markSoldByIds(postIds, client) {
  ...
  static async updateSoldStatus(id) {
    const { rows } = await pool.query(
      `
        UPDATE gallery_posts
        SET sold=true
