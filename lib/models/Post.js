...
module.exports = class Post {
  ...
  static async postNewPost(
    ...
    const { rows } = await pool.query(
      'INSERT INTO gallery_posts (title, description, image_url, category, price, creator_id, public_id, num_imgs) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
      [
        title,
        description,
        image_url,
  ...
  static async getById(post_id) {
    const { rows } = await pool.query(
      ...
      SELECT * 
      FROM gallery_posts
      LEFT JOIN creators on creators.id = gallery_posts.creator_id
      WHERE id=$1 
      `,
      [post_id]
    );
