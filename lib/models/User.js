...
module.exports = class User {
  ...
  constructor(row) {
    ...
    this.passwordResetTokenVersion = row.password_reset_token_version;
  }
  ...
  // Returns null for an unknown email. Callers that treat "no such user" as a normal
  // outcome (looking up a sale buyer) use this; getByEmail keeps throwing for the
  // auth paths that treat a miss as an error.
  ...
  static async updatePassword(userId, passwordHash) {
    const { rows } = await pool.query(
      ...
      UPDATE users_admin
      SET password_hash = $2,
          password_reset_token_version = password_reset_token_version + 1
      WHERE id = $1
      RETURNING *
      `,
      [userId, passwordHash],
  ...
  static async incrementPasswordResetTokenVersion(userId) {
    const { rows } = await pool.query(
      `
      UPDATE users_admin
      SET password_reset_token_version = password_reset_token_version + 1
      WHERE id = $1
      RETURNING *
      `,
      [userId],
