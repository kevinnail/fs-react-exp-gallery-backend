...
module.exports = class Gallery {
  ...
  static async getGalleryPosts() {
    const { rows } = await pool.query(
      'SELECT * FROM gallery_posts WHERE hide = FALSE AND is_deleted = FALSE ORDER BY created_at DESC',
    );
  ...
  static async searchGalleryPosts(searchTerm) {
    const { rows } = await pool.query(
      ...
      WHERE is_deleted = FALSE AND (
        title ILIKE $1 OR title ILIKE $2 OR title ILIKE $3 OR title ILIKE $4
  ...
  static async getGalleryPostById(id) {
    const { rows } = await pool.query(
      'SELECT * FROM gallery_posts WHERE id=$1 AND is_deleted = FALSE',
      [id],
    );
