...
module.exports = class Message {
  ...
  static async insert({ userId, messageContent, conversationId = null, isFromAdmin = false }) {
    ...
    // If no conversationId provided, check for existing conversation for this user
    if (!actualConversationId) {
      // Look for an existing conversation for this user
      const existingConv = await pool.query(
        'SELECT conversation_id FROM messages WHERE user_id = $1 ORDER BY sent_at ASC LIMIT 1',
        [userId],
      );
      if (existingConv.rows.length > 0 && existingConv.rows[0].conversation_id) {
        actualConversationId = existingConv.rows[0].conversation_id;
      } else {
        // No existing conversation, create a new conversation ID
