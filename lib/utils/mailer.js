...
async function sendNewPostEmail({ to, post }) {
  ...
  const postUrl = `${process.env.FRONTEND_URL}/${post.id}`;

  const templatePath = path.join(process.cwd(), 'lib', 'emailTemplates', 'newPostEmail.html');
...
async function sendPasswordResetEmail(to, resetToken) {
  ...
  const templatePath = path.join(process.cwd(), 'lib', 'emailTemplates', 'resetPasswordEmail.html');
  let html = fs.readFileSync(templatePath, 'utf8');
...
async function sendMassEmail({ to, subject, message }) {
  if (!process.env.MAIL_FROM) throw new Error('MAIL_FROM environment variable is required');
  const homePageUrl = process.env.FRONTEND_URL;
  const instagramUrl = 'https://www.instagram.com/stresslessglass';
  ...
  // admin-authored plain text: escape it, then turn newlines into line breaks
  const messageBody = escapeHtml(message ?? '').replace(/\r\n|\r|\n/g, '<br />');

  const templatePath = path.join(process.cwd(), 'lib', 'emailTemplates', 'massEmail.html');
  let html = fs.readFileSync(templatePath, 'utf8');

  html = html.replace('{{subject}}', escapeHtml(subject ?? ''));
  ...
  // eslint-disable-next-line no-console
  console.log(`Mass email sent to: ${to}`);
}
