...
const storage = new CloudinaryStorage({
  ...
  params: {
    ...
    allowed_formats: ['jpg', 'png', 'jpeg', 'gif', 'mp4', 'mov'], // optional, specify allowed formats
...
module.exports = Router()
  .get('/download-inventory-csv', async (req, res, next) => {
    try {
      const data = await User.getByEmail(req.user.email);
      await data.getGalleryPosts();
      ...
      const csv = parse(data.galleryPosts, opts);
