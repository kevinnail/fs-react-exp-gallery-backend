...
const { notifyUsersNewPost } = require('../services/postEmailService');
const { enqueue } = require('../jobs/emailQueue');
const { sendMassEmailToCustomers } = require('../services/adminEmailService');

//# Configure S3 client
const { S3Client, PutObjectCommand, DeleteObjectCommand } = require('@aws-sdk/client-s3');
const SalesOrder = require('../models/SalesOrder.js');
const { sendTrackingEmail } = require('../utils/mailer.js');
...
module.exports = Router()
  ...
  .post('/mass-email', async (req, res, next) => {
    try {
      ...
      const result = await sendMassEmailToCustomers({ subject, message });
  ...
  .post('/sales', async (req, res, next) => {
    try {
      const { buyerEmail, items, shippingCost, tracking } = req.body;
      ...
      const everyItemIsValid = items.every(
        (item) => item && item.postId && Number.isFinite(Number(item.price)),
      );
      if (!everyItemIsValid) {
        return res.status(400).json({ message: 'each item requires a postId and a numeric price' });
      }

      const shipping = shippingCost === undefined || shippingCost === '' ? 0 : Number(shippingCost);
      if (!Number.isFinite(shipping) || shipping < 0) {
        return res.status(400).json({ message: 'shippingCost must be a number of at least 0' });
      }
      ...
      // one order, its items, and the sold flags, in a single transaction
      const order = await SalesOrder.createOrder({
        buyerId: buyer.id,
        items: items.map((item) => ({ postId: item.postId, price: item.price })),
        shippingCost: shipping,
        tracking: tracking || null,
      });

      // Emit sale-created websocket event with the nested order for UI hydration.
      // Re-read the order so the items carry the post title and image the buyer's
      // account page renders; the insert only returns the item columns themselves.
      if (global.wsService && typeof global.wsService.emitSaleCreated === 'function') {
        const hydrated = await SalesOrder.getOrderById(order.id);
        global.wsService.emitSaleCreated({
          type: 'sale',
          orderId: hydrated.id,
          userId: hydrated.buyer_id,
          shippingCost: hydrated.shipping_cost,
          trackingNumber: hydrated.tracking_number,
          isPaid: hydrated.is_paid,
  ...
  .post('/', async (req, res, next) => {
    try {
      ...
      enqueue({ post }, notifyUsersNewPost);
  ...
  .put('/sale-pay-status/:id', async (req, res, next) => {
    try {
      ...
      const result = await SalesOrder.updatePaidStatus(orderId, isPaid);
      if (!result) {
  ...
  .put('/:id/tracking', async (req, res, next) => {
    try {
      ...
      const result = await SalesOrder.updateTracking(orderId, trackingNumber);
      if (!result) {
        return res.status(404).json({ error: 'Order not found' });
      }
      ...
      const { email } = await User.getEmailById(result.buyer_id);
      const auctionOrPost = 'post';
      await sendTrackingEmail(auctionOrPost, email, trackingNumber);
