...
router
  ...
  .get('/', async (req, res, next) => {
    try {
      ...
      if (!profile) {
        return res.json({ profile: null, address: null });
      }
  ...
  .put('/', async (req, res, next) => {
    try {
      ...
      // Update profile (name, avatar, notifications)
      ...
      // Handle address with smart validation
      const addressFields = [addressLine1, city, state, postalCode];
      const hasAnyAddressData = addressFields.some((field) => field && field.trim());
      const hasAllRequiredAddressData = addressFields.every((field) => field && field.trim());

      let address = null;

      if (hasAnyAddressData && !hasAllRequiredAddressData) {
        return res.status(400).json({
          message:
            'Please complete all required address fields (street, city, state, postal code) or leave them all blank',
        });
      }

      if (hasAllRequiredAddressData) {
        address = await Address.upsertByUserId(req.user.id, {
          addressLine1,
  ...
  .put('/welcome-message/:userId', async (req, res, next) => {
    try {
      const userId = req.params.userId;
      const { show_welcome } = await Profile.removeWelcomeMessage(userId);

      res.json({ showWelcome: show_welcome });
    } catch (e) {
      next(e);
    }
  })
  ...
  .post('/images', upload.none(), async (req, res, next) => {
    try {
      ...
      const profile = await Profile.upsertByUserId(req.user.id, {
        ...
        imageUrl: image_url,
        sendEmailNotifications: req.body.sendEmailNotifications,
      });
      res.json(profile);
    } catch (e) {
      next(e);
