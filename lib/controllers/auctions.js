...
const { notifyUsersNewAuction } = require('../services/auctionEmailService.js');
const { enqueue } = require('../jobs/emailQueue.js');
...
module.exports = Router()
  ...
  // GET live auctions (must stay above /:id) ///////////////////////////////////////////
  .get('/live', async (req, res, next) => {
    try {
      const auctions = await Auction.getLive();
      res.json(auctions);
    } catch (e) {
      next(e);
    }
  ...
  .get('/user-auctions/:id', [authenticate], async (req, res, next) => {
    try {
  ...
  .post('/upload', [authenticate, authorize], upload.array('imageFiles'), async (req, res) => {
    try {
  ...
  .post('/', [authenticate, authorize], async (req, res, next) => {
    try {
      ...
      if (global.wsService) {
        global.wsService.emitAuctionCreated(auction);
      }

      enqueue({ auction }, notifyUsersNewAuction);
  ...
  .put('/:id/paid', [authenticate, authorize], async (req, res, next) => {
    try {
      const { id } = req.params;
  ...
  // PUT update tracking # /////////////////////////////////
  .put('/:id/tracking', [authenticate, authorize], async (req, res, next) => {
    try {
      const auctionOrPost = 'auction';
      const { id } = req.params;
