...
app.use('/api/v1/user-sales', authenticate, require('./controllers/user-sales'));
app.use('/api/v1/messages', require('./controllers/messages'));
app.use('/api/v1/auctions', require('./controllers/auctions'));
app.use('/api/v1/bids', require('./controllers/bids'));
app.use('/api/v1/auction-notifications', require('./controllers/auctionNotifications'));
...
app.use((req, res, next) => {
  if (req.path.startsWith('/api/v1')) {
    ...
    try {
      ...
      // Add a meta tag with the nonce for React/Emotion to use
      html = html.replace(
        '<head>',
        `<head>\n    <meta name="csp-nonce" content="${res.locals.nonce}">\n    <script nonce="${res.locals.nonce}">window.__NONCE__ = "${res.locals.nonce}";</script>`,
      );
