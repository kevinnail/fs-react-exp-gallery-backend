...
async function sweepExpiredAuctions() {
  try {
    ...
    try {
      await ErrorLog.log(err.message, 'sweepExpiredAuctions');
    } catch (logErr) {
...
function scheduleAuctionEnd(auctionId, endTime) {
  ...
  const timer = setTimeout(async () => {
    try {
      ...
      try {
        await ErrorLog.log(err.message, `scheduleAuctionEnd auctionId: ${auctionId}`);
      } catch (logErr) {
        console.error('Failed to log error:', logErr);
      }
...
async function initAuctionTimers() {
  // safety sweep every minute from 5:00-5:30 PM Pacific daily
  ...
  try {
    ...
    try {
      await ErrorLog.log(err.message, 'initAuctionTimers');
    } catch (logErr) {
      console.error('Failed to log error:', logErr);
    }
