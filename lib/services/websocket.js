        ...
        try {
          ...
          if (socket.isAdmin) {
            try {
              ...
              // Prefer an actual customer message, fallback to the userId from the event
              const customerMessage = conversationMessages.find((msg) => !msg.isFromAdmin);

              const resolvedUserId = customerMessage ? customerMessage.userId : data.userId; // fallback when admin sends the very first message
