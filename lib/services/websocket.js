...
class WebSocketService {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('send_message', async (data) => {
        try {
          ...
          // Email notification: send for both admin and customer messages
          try {
            const Message = require('../models/Message');
            let resolvedUserId;
            if (socket.isAdmin) {
              // For admin messages, prefer an actual customer message, fallback to the userId from the event
              ...
              resolvedUserId = customerMessage ? customerMessage.userId : data.userId;
            } else {
              // For customer messages, recipient is the admin
              resolvedUserId = process.env.ADMIN_ID;
            }

            if (!resolvedUserId) {
              // eslint-disable-next-line
              console.warn('Email skip: unable to resolve recipient userId', {
                conversationId: data.conversationId,
                data,
              });
              return;
            ...
            const recipient = await User.getEmailById(resolvedUserId);
            const result = await sendMessageNotificationEmail({ user: recipient, message });

            if (result?.sent) {
              // eslint-disable-next-line
              console.log('Email sent for message', {
                to: recipient.email,
                conversationId: data.conversationId,
              ...
              // eslint-disable-next-line
              console.log('Email not sent for message', {
                to: recipient.email,
                conversationId: data.conversationId,
                messageId: message.id,
                isFromAdmin: socket.isAdmin,
  ...
  // Payments, tracking and shipping live on the order, so these events are order-scoped.
  // Standard payload shape:
  // sale-paid: { type: 'sale', orderId, userId, isPaid }
  // sale-tracking-info: { type: 'sale', orderId, userId, trackingNumber }
  // sale-created: { type: 'sale', orderId, userId, shippingCost, trackingNumber, isPaid, items }

  buildSalePaidPayload(orderOrPayload) {
    if (orderOrPayload.orderId) return orderOrPayload;
    return {
      type: 'sale',
      orderId: orderOrPayload.id,
      userId: orderOrPayload.buyer_id,
      isPaid: orderOrPayload.is_paid,
    };
  }

  buildSaleTrackingPayload(orderOrPayload) {
    if (orderOrPayload.orderId) return orderOrPayload;
    return {
      type: 'sale',
      orderId: orderOrPayload.id,
      userId: orderOrPayload.buyer_id,
      trackingNumber: orderOrPayload.tracking_number,
    };
  }

  buildSaleCreatedPayload(orderOrPayload) {
    if (orderOrPayload.orderId) return orderOrPayload;
    return {
      type: 'sale',
      orderId: orderOrPayload.id,
      userId: orderOrPayload.buyer_id,
      shippingCost: orderOrPayload.shipping_cost,
      trackingNumber: orderOrPayload.tracking_number,
      isPaid: orderOrPayload.is_paid,
      items: orderOrPayload.items || [],
    };
  }

  emitSalePaid(orderOrPayload) {
    const payload = this.buildSalePaidPayload(orderOrPayload);
  ...
  emitSaleTrackingInfo(orderOrPayload) {
    const payload = this.buildSaleTrackingPayload(orderOrPayload);
  ...
  emitSaleCreated(orderOrPayload) {
    const payload = this.buildSaleCreatedPayload(orderOrPayload);
